-- Fix profile role audit fallback lookup for user_company_access.
-- That table does not have created_at, so use granted_at only.
  ...
  IF TG_OP = 'UPDATE' AND OLD.role IS DISTINCT FROM NEW.role THEN
    ...
    v_company_id := COALESCE(
      ...
      (
        ...
        ORDER BY uca.granted_at DESC NULLS LAST
        LIMIT 1
      ),
      (
