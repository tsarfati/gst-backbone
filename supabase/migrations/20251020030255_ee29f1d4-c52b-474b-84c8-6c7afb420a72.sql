-- Fix the late punch-out logic in create_time_card_from_punch trigger
-- The correct behavior is:
-- If count_late_punch_out is enabled:
--   - Within grace period: don't count late time
--   - Beyond grace period: count ALL late time (including the grace period)
-- If count_late_punch_out is disabled:
--   - Never count late time
  ...
  IF NEW.punch_type = 'punched_out'::public.punch_status THEN
    ...
    IF punch_in_record IS NOT NULL THEN
      ...
      IF job_shift_start IS NOT NULL AND job_shift_end IS NOT NULL THEN
        ...
        -- Late punch-out handling (FIXED LOGIC)
        IF NEW.punch_time > shift_end_datetime THEN
          IF count_late_punch THEN
            -- Only count late time if beyond grace period, but include grace period in total
            IF NEW.punch_time > (shift_end_datetime + (late_grace_minutes || ' minutes')::interval) THEN
              -- Beyond grace period - count ALL late time including grace period
              adjusted_punch_out := NEW.punch_time;
            ELSE
              -- Within grace period - don't count
              adjusted_punch_out := shift_end_datetime;
            END IF;
          ELSE
            -- Setting disabled - never count late time
            adjusted_punch_out := shift_end_datetime;
          END IF;
        END IF;
      END IF;
