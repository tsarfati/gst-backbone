-- Create the trigger function and trigger to clean up job budgets when cost codes are removed
...
-- Create the trigger
DROP TRIGGER IF EXISTS job_budget_cleanup_trigger ON cost_codes;
CREATE TRIGGER job_budget_cleanup_trigger
  AFTER UPDATE OR DELETE ON cost_codes
  FOR EACH ROW
  EXECUTE FUNCTION cleanup_job_budget_on_cost_code_removal();