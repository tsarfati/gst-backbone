-- Update time_cards table to set default status as 'approved' for punch clock entries
-- Add a column to track if the time card was created via punch clock
ALTER TABLE time_cards 
ADD COLUMN IF NOT EXISTS created_via_punch_clock BOOLEAN DEFAULT false;

-- Update the trigger function to mark punch clock entries as approved
  ...
  IF NEW.punch_type = 'punched_out'::public.punch_status THEN
    ...
    IF punch_in_record IS NOT NULL THEN
      ...
      INSERT INTO public.time_cards (
        ...
        NEW.photo_url,
        NEW.notes,
        'approved', -- Auto-approve punch clock entries
        true -- Mark as created via punch clock
      );
    END IF;
  END IF;
