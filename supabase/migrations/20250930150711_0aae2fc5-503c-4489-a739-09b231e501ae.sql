...
-- Policies: admins/controllers can manage; PMs can manage; users can view within their companies
CREATE POLICY "Admins and controllers can manage pin employee timecard settings"
ON public.pin_employee_timecard_settings
FOR ALL
USING (
  company_id IN (
...
CREATE POLICY "Project managers can manage pin employee timecard settings"
ON public.pin_employee_timecard_settings
FOR ALL
USING (public.has_role(auth.uid(), 'project_manager'))
WITH CHECK (public.has_role(auth.uid(), 'project_manager'));

CREATE POLICY "Users can view pin employee timecard settings for their companies"
ON public.pin_employee_timecard_settings
FOR SELECT
USING (
  company_id IN (
