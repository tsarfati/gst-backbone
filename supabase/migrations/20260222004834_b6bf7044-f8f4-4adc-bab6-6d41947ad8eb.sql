DO $$
DECLARE
  target_company_id CONSTANT uuid := 'f64fff8d-16f4-4a07-81b3-e470d7e2d560';
  grantor_user_id CONSTANT uuid := 'dcdfec98-5141-4559-adb2-fe1d70bfce98';
BEGIN
  IF EXISTS (SELECT 1 FROM public.companies WHERE id = target_company_id) THEN
    ...
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = grantor_user_id) THEN
      INSERT INTO public.user_company_access (user_id, company_id, role, granted_by, is_active)
      SELECT user_id, target_company_id, 'employee', grantor_user_id, true
      FROM (
        VALUES
          ('8855c1c0-db14-4819-b106-1562b35505f4'::uuid),
          ('bbc0b8cd-ece3-4005-9cc3-d4cbd2bbf593'::uuid)
      ) AS employees(user_id)
      WHERE EXISTS (SELECT 1 FROM auth.users WHERE id = employees.user_id)
