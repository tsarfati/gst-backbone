-- 1) Update payment posting function to post on insert (do not wait for 'cleared')
...
BEGIN
  ...
  -- Get company_id from the invoice linked to this payment (if any)
...
-- 2) Triggers to post on payment creation
DROP TRIGGER IF EXISTS trg_payments_post_journal ON public.payments;
CREATE TRIGGER trg_payments_post_journal
AFTER INSERT ON public.payments
FOR EACH ROW
EXECUTE FUNCTION public.create_payment_journal_entry();

-- 3) Triggers to keep chart_of_accounts.current_balance in sync
DROP TRIGGER IF EXISTS trg_account_balance_after_insupd ON public.journal_entry_lines;
CREATE TRIGGER trg_account_balance_after_insupd
AFTER INSERT OR UPDATE ON public.journal_entry_lines
FOR EACH ROW
EXECUTE FUNCTION public.update_account_balance();

DROP TRIGGER IF EXISTS trg_account_balance_after_delete ON public.journal_entry_lines;
CREATE TRIGGER trg_account_balance_after_delete
AFTER DELETE ON public.journal_entry_lines
FOR EACH ROW
EXECUTE FUNCTION public.update_account_balance();

-- 4) Optional: enforce balancing of entries when editing lines
...
-- 5) One-time backfill: recompute balances for all accounts
DO $$
DECLARE rec RECORD;
BEGIN
