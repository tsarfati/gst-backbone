...
const getCandidateWebhookSecrets = (): string[] => {
  const rawValues = [
    ...
    Deno.env.get("TASK_EMAIL_WEBHOOK_SECRET") || "",
  ];
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: channel, error: channelError } = await supabase
      ...
      .select("task_id, company_id, tracking_email, created_by")
    ...
    if (!channel) {
      return new Response(JSON.stringify({ ok: true, ignored: "No task tracking email matched" }), {
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    ...
    if (providerMessageId) {
      const { data: existingMessageRows } = await supabase
        .from("task_email_messages")
        .select("id, body_text, body_html")
        .eq("task_id", channel.task_id)
    ...
    if (!existingInboundMessageId) {
      const { error: insertMessageError } = await supabase.from("task_email_messages").insert({
    ...
    if (uploadedBy && inboundAttachments.length > 0) {
      for (const attachment of inboundAttachments) {
        ...
        try {
          ...
          const objectPath = `${channel.company_id}/${channel.task_id}/email/${providerMessageId || emailId || Date.now().toString()}-${filename}`;
          ...
          if (uploadError) {
            console.error("Failed to upload inbound task email attachment:", uploadError);
            continue;
          }

          const { data: publicData } = supabase.storage.from(TASK_ATTACHMENTS_BUCKET).getPublicUrl(objectPath);
          const { error: attachmentInsertError } = await supabase.from("task_attachments").insert({
            task_id: channel.task_id,
            file_name: filename,
          ...
          if (attachmentInsertError) {
            console.error("Failed to insert inbound task email attachment row:", attachmentInsertError);
            continue;
          }
          attachmentCount += 1;
        } catch (attachmentError) {
          console.error("Inbound task email attachment processing failed:", attachmentError);
        }
      }
    }
    ...
    console.error("receive-task-email-webhook error:", error);
