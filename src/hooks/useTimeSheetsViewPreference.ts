...
import { TimeSheetsViewType } from '@/components/TimeSheetsViewSelector';

interface TimeSheetsViewPreference {
  currentView: TimeSheetsViewType;
  defaultView: TimeSheetsViewType;
}

export function useTimeSheetsViewPreference(storageKey: string, initialView: TimeSheetsViewType = 'list') {
  const [viewPreference, setViewPreference] = useState<TimeSheetsViewPreference>(() => {
  ...
  const setCurrentView = (view: TimeSheetsViewType) => {
