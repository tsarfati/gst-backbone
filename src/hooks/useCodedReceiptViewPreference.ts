...
interface CodedReceiptViewPreference {
  currentView: CodedReceiptViewType;
  defaultView: CodedReceiptViewType;
}

export function useCodedReceiptViewPreference(storageKey: string, initialView: CodedReceiptViewType = 'list') {
  const [viewPreference, setViewPreference] = useState<CodedReceiptViewPreference>(() => {
  ...
  const setCurrentView = (view: CodedReceiptViewType) => {
