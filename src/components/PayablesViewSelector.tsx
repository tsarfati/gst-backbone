...
export type PayablesViewType = "list" | "compact" | "super-compact";

interface PayablesViewSelectorProps {
  currentView: PayablesViewType;
  onViewChange: (view: PayablesViewType) => void;
  onSetDefault: () => void;
  isDefault?: boolean;
  className?: string;
}
...
const views = [
  ...
  { type: 'compact' as const, icon: AlignJustify, label: 'Compact List' },
  { type: 'super-compact' as const, icon: Grid3X3, label: 'Super Compact List' }
];

export default function PayablesViewSelector({ 
  currentView, 
  onViewChange, 
  onSetDefault, 
  isDefault = false,
  className = "" 
}: PayablesViewSelectorProps) {
